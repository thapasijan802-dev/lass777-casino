      ...
      <div className="relative w-full max-w-md rounded-2xl bg-[#0e131d] border border-amber-500/40 p-6 sm:p-8 shadow-gold-glow-lg text-white">
        ...
        <div className="text-center mb-6">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-black uppercase mb-2">
            ...
            <span>9Casino VIP Membership</span>
          </div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            {authModalTab === 'login'
              ? 'Enter your credentials to access your crypto casino wallet'
              : 'Register in 10 seconds and receive a $20 FREE bonus chip!'}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 p-1 bg-slate-900/90 rounded-xl border border-slate-800 mb-6">
          <button
            ...
            className={`py-2 text-xs font-black uppercase tracking-wider rounded-lg transition-all ${
              authModalTab === 'login'
                ? 'bg-emerald-500 text-black shadow-[0_0_12px_rgba(0,231,1,0.3)]'
        ...
        {error && (
          <div className="mb-4 p-3 rounded-xl bg-red-950/80 border border-red-500/50 text-red-200 text-xs text-center font-bold">
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-[#141b2b] border border-slate-700 text-white text-sm focus:outline-none focus:border-emerald-400 transition-colors"
                />
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="player@9casino.com"
                className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-[#141b2b] border border-slate-700 text-white text-sm focus:outline-none focus:border-emerald-400 transition-colors"
              />
            </div>
          </div>
          ...
          {authModalTab === 'register' && (
            <div>
              <label className="block text-xs font-bold text-emerald-400 uppercase mb-1">Promo Code</label>
              <div className="relative">
                <Gift className="absolute left-3 top-3 w-4 h-4 text-emerald-400" />
                <input
                  type="text"
                  value={promoCode}
                  onChange={(e) => setPromoCode(e.target.value)}
                  placeholder="FREE20"
                  className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-[#141b2b] border border-emerald-500/50 text-emerald-300 font-mono font-bold text-sm focus:outline-none focus:border-emerald-400 transition-colors"
                />
              </div>
            </div>
          )}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-2 py-3 rounded-xl bg-gradient-to-r from-emerald-400 to-[#00e701] text-black font-black uppercase tracking-wider text-sm shadow-[0_0_20px_rgba(0,231,1,0.35)] hover:brightness-110 active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
          >
            {loading ? (
              <span className="inline-block animate-spin">🌀</span>
            ) : (
              <>
                <span>{authModalTab === 'login' ? 'ENTER 9CASINO' : 'CLAIM $20 & PLAY'}</span>
        ...
        <div className="mt-6 pt-5 border-t border-slate-800/80">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className="py-2 px-3 rounded-lg bg-slate-800/80 hover:bg-slate-700/80 border border-slate-700 text-xs font-bold text-emerald-400 flex items-center justify-center gap-1 transition-colors"
            >
